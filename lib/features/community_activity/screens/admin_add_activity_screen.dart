...
class _AdminAddActivityScreenState extends State<AdminAddActivityScreen> {
  ...
  Future<void> _saveActivity() async {
    ...
    try {
      // 1. Convert Address to Lat/Lon via Nominatim
      final coordinates = await _nominatimService.getCoordinates(_addressController.text);

      if (coordinates == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Could not find location for this address. Please try a clearer address.')),
          );
        }
        setState(() {
          _isSaving = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Admin: Add Activity')),
      body: _isSaving
          ? const Center(child: Column(
            ...
            children: [
              ...
              SizedBox(height: 16),
              Text("Geocoding address via OpenStreetMap..."),
            ],
          ))
